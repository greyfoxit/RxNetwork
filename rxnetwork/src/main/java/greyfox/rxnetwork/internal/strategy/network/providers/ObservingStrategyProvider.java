 ...
 * @author Radek Kozak
 */
interface ObservingStrategyProvider<T extends ObservingStrategy> {

  /**
   * Implement this method to determine under what condition your provider can {@link #provide()}
   * concrete {@link ObservingStrategy}.
   *
   * @return {@code true} if concrete {@link ObservingStrategy} can be provided
   * for given criteria, {@code false} if not
   */
  boolean canProvide();

  /**
   * Implement this to return concrete {@link ObservingStrategy}.
   *
   * @return {@link ObservingStrategy}
   */
  T provide();
}
